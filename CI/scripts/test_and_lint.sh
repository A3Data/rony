...
# If no arguments are supplied, all modules will be tested and lint checked
# If module name is supplied as an argument, it will be tested and lint checked
if [ $# -eq 0 ]
  then
    # Iterating over all module templates
    for d in ../rony/module_templates/*/ ; do
        module_name=$(basename "$d")
        test_and_lint_module $module_name
    done
  else
    test_and_lint_module $1
fi